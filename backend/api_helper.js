...
async function savePopular(date){
  ...
  try{
    if(date.date === undefined){
      ...
      api_call = await fetch(`https://www.flickr.com/services/rest/?method=flickr.interestingness.getList&date=${date.date}&format=json&nojsoncallback=1&per_page=3&api_key=${KEY}`,
      {method: "GET",
      headers: {}});
    }
